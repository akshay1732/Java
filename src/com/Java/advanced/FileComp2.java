	...
	public class FileComp2 {	
		...
		for (int a=0;a<rowcount1;a++) {
			...
			for(int y=0;y<numberofkeys;y++) {
				...
				cellval2[y] =table2.get(a).get(keyval[y]);
			...
			for(int col=0;col<cellval1.length;col++) {
				...
				if(cellval1[col].equals(cellval2[col])) {		
						...
						failedcols1.put(keyval[col], cellval1[col]);
						failedcols2.put(keyval[col], cellval2[col]);
					}
		...
		if(mainflag) {
			...
			keyresultrow.createCell(3).setCellValue("All Columns Matched");
		}
		else {
			...
			if(!rowexist) {
				keyresultrow.createCell(2).setCellValue("Less Constituents");
				keyresultrow.createCell(3).setCellValue("Row Exist in File 1 but Missing in File 2");
			}
			keyresultrow.createCell(2).setCellValue("Mismatched");
			int falsecount = failedcols1.size();
			int matched = numberofkeys-	falsecount;
			keyresultrow.createCell(3).setCellValue(matched+" columns matched//"+falsecount+" Mismatched");		
		}
